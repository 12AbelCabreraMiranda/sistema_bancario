<?php
	...
	if (empty($_POST['name'])){
		...
		$consultaVerificacion = ("SELECT * FROM chequeras where numero_de_cuenta='$cuenta_numCuenta'");
		$resultadoVerificacion = $con->query($consultaVerificacion);
		if($row = $resultadoVerificacion->fetch_assoc()){      
			$messages[] = "El numero de cuenta existe.";			
			}else{
